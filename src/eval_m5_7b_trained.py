#!/usr/bin/env python3
"""7B 训练后 × 多prompt投票：验证训练+推理叠加（3B 叠加+15%，7B 可能 85%+）"""
...
BASE_MODEL = str(PROJECT / 'models' / 'qwen2.5-coder-7b-instruct')
LORA = str(PROJECT / 'checkpoints' / 'grpo_ea_g8')  # E-A G=8 训练后的 7B
SPIDER = str(PROJECT / 'data' / 'spider_data')
OUT = PROJECT / 'outputs' / 'eval_m5_7b_trained'


def build_prompt_variants(question: str, ddl: str) -> list:
    """3 种 prompt 视角（与 M5 相同）"""
...
def main():
    print("7B-trained × multi-prompt voting")
    ...
    print(f"\n=== 7B-TRAINED × VOTING RESULT ===")
    print(f"Match: {match_count}/100 ({rate:.1%})")
    print(f"Time:  {elapsed:.0f}s")
    print(f"7B zero-shot: 81% | 7B+vote: 85% | 3B train+vote: 65%")

    OUT.mkdir(parents=True, exist_ok=True)
    with open(OUT / 'summary.json', 'w') as f:
        json.dump({'method': 'm5_7b_trained_vote', 'match_count': match_count, 'match_rate': rate,
                   'elapsed_seconds': round(elapsed, 1), 'lora': str(LORA)}, f, indent=2)
