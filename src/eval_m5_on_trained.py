#!/usr/bin/env python3
"""M5-叠加: 多prompt投票 × 训练后模型（3B checkpoint-25，最佳40%）"""
...
LORA = str(PROJECT / 'checkpoints' / 'grpo_3b_3lvl' / 'checkpoint-25')
SPIDER = str(PROJECT / 'data' / 'spider_data')
OUT = PROJECT / 'outputs' / 'eval_m5_3b_trained'
...
def main():
    print("M5-stack: Multi-prompt voting on TRAINED 3B (ckpt-25)")
    ...
    print(f"\n=== M5-STACK RESULT (trained 3B + voting) ===")
    print(f"Match: {match_count}/100 ({rate:.1%})")
    print(f"Time:  {elapsed:.0f}s")
    print(f"3B baseline: 34% | 3B trained-only: 40% | 7B M5: 85%")

    OUT.mkdir(parents=True, exist_ok=True)
    with open(OUT / 'summary.json', 'w') as f:
        json.dump({'method': 'm5_stack_3b_trained', 'match_count': match_count, 'match_rate': rate,
