#!/usr/bin/env python3
"""M2: 低温自一致性投票 — temp=0.2, 8候选, 执行结果投票"""
...
OUT = PROJECT / 'outputs' / 'eval_lowtemp_vote'
...
def main():
    print(f"M2: Low-temp voting ({N_CAND} candidates, temp={TEMP})")
    ...
    with open(OUT / 'summary.json', 'w') as f:
        json.dump({'method': f'lowtemp_vote_{N_CAND}_t{TEMP}', 'match_count': match_count, 'match_rate': rate,
