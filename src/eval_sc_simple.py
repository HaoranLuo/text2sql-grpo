#!/usr/bin/env python3
"""Self-consistency: 5 diverse samples via sampling, majority vote."""
...
print(f'Self-Consistency: {N} samples with sampling')
print('Loading model...')
...
model.eval()
print(f'Model loaded. VRAM: {torch.cuda.memory_allocated(0)/1024**3:.1f} GiB')
...
for i, item in enumerate(items):
    ...
    # Generate N diverse samples (sequential, each with temperature sampling)
    exec_results = []
    for _ in range(N):
...
print(f'\n=== SELF-CONSISTENCY ({N} samples, temp=0.8) ===')
...
with open(OUT / 'summary.json', 'w') as f:
    json.dump({'method': f'self_consistency_{N}', 'match_count': match_count, 'match_rate': rate, 'elapsed_seconds': round(elapsed,1)}, f, indent=2)
