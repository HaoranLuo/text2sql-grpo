...
| **L6** 新组件方法（C 方案） | 执行感知 memory 奖励 / 动态奖励课程 / 多视角 vav 理论分析；止损线：组件 2 次验证无提升即转综述或并入实证 | CCF-A/B、SCI 1-2 区 | ★★★★★·3-6 月 | 未开始（依赖 L4） |
...
- **当前**：L0 审计完成（2026-08-12，根因闭合），**立即执行 4 项修复并重评确认 ≈85.0%**（见 §8 P2 fix_plan）；随后归档固化两套口径脚本、更新 NEXT_STEPS.md 移除多实例库待查项、删旧备份；再跑 M1 校准探针确认生成侧超参；最后产出三口径对照表 85.88/85.0/76.6 + 口径说明段。
- **随后**：P0 #1 推理蒸馏→SFT（主战役）→ 数据卫生 P0 #3 → L2 投票升级官方口径验证 → L1 技术报告。
...
- **论文分层 L0-L6**（PAPER_PLAN.md / PROJECT_GUIDE §7）：L0 口径 → L6 新组件，L7/L8 为对话中的拓展讨论（未入正式计划）
