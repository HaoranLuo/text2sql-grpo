...
| **grpo_t22 finer从SFT起（2026-08-13，仍负）** | best 自定义 54.8% / 官方 **50.2%** | 稠密奖励单独不足以阻止回吐（-9pp,仅比 T2.1 +1pp）；exec 88.8→84.0 同型退化。**Gate-2 决策点**：T2.3 全剂量（DAPO 零优势组过滤/clip-higher/SFT 混合/lr 3e-7/熵早停）vs 转向投票放大（Phase 3 已并行启动） |
| **phase3 5p投票×SFT（2026-08-13 晚，胜果）** | 自定义 **73.4%** / 官方 **70.1%** | 单次 60.8→投票 70.1 = **+9.3pp 官方**；难度 89.9/73.9/59.4/41.0；**3B(蒸馏SFT+投票)首次超过 7B 基线官方 67.5%**；验证"训练抬基线→投票放大"主线逻辑 |
| **grpo_t23 dr_grpo（2026-08-14 凌晨，三连败封盘）** | best 自定义 53.7% / 官方 **49.3%** | T2.1 49.4 / T2.2 50.2 / T2.3 49.3 vs SFT 60.8%——Dr.GRPO(token-mean)+去std+lr1e-6 全部无效,同型退化(exec 88.8→82.7)。**判定: RL 对 3B 从强 SFT 起点不 work,训练侧(RL)封盘**;三连负结果是 L5 实证论文素材。转向:投票升级 + 蒸馏加料 SFT v2(3764 条教案已备) |
| **mpev_sft100 官方复测（2026-08-14 凌晨）** | vav_self 85% / custom 73% / **官方 64.0%** | 21pp 自评-官方差(多实例库严格性,T0.4 归因印证);**D4 判定:官方 64.0% < 5p 全量官方 70.1% → 回退 5p,MPEV 不采用**。口径敏感性(同一投票法自评 85 官方 64)是 L7 论文重要发现 |
| **maj_diag 诊断曲线（2026-08-14 凌晨，L7核心）** | maj@K 88/88/87/85;Pass@K 91→98 | **饱和@4 且 K 大微降(票数稀释)**;K=32 时 98% 题有正确候选但 maj 仅 85 = **投票失效=选择失效(13pp 缺口)**;绝对多数命中 98.5%;Self-BLEU 0.64→0.88 |
| **sft_v2 加料（2026-08-14 凌晨）** | 自定义 66.8% / 官方 **63.5%** | 3.8×QC 教案 → **剂量响应 +2.7pp**(对数边际递减:3.8×数据换 2.7pp);exec 88.8→90.6;投票放大已排队(预期 ~72.5%) |
| **5p×v2 投票（2026-08-14）** | 自定义 72.7% / 官方 **68.7%** | 增益缩水 +9.3→+5.2:**投票收益随基座质量递减**(多样性瓶颈)——L7 论文第二块机制发现 |
| **5p 高温 T=1.0×v2（2026-08-14）** | 官方 **66.1%** | 高温反降 2.6pp:采样噪声多样性 < 候选质量损失——**多样性假说否证**(第三次印证结论 11) |
| **5p+few-shot 6视角×v2（2026-08-14）** | 官方 **68.9%** | 微增益 +0.2pp(噪声内):视角多样性边际也递减(与 5p→7p 零提升一致)。**投票升级战役收官:70.1%(v1+5p)为最优** |
| **mixvav 十票混合投票（2026-08-15，未赢）** | 自定义 73.9% / 官方 **68.3%** | v1+v2 各 5 视角共 10 票, vav 单实例裁决: 自定义+0.5pp 但官方-0.6pp(vs 同 run v1 单臂 68.9);简单多数 166 平票崩(72.5%)。交叉 oracle 上限 77.9%(0-B 推演)未兑现——单实例分组的官方错位第三证 |
| **质心过滤全量（2026-08-15，负）** | 自评 82.0% / 官方 **68.9%** | 基线 maj@16 官方 70.0% vs 过滤臂 68.9%(-1.1pp):**自评+0.5pp 官方反向**——口径错位第四证;G-B0 FAIL, 过滤退场为 L7 数据点 |
| **maj@K×温度网格（2026-08-15，L7 核心）** | Pass@K 84→95%;maj@K 67-72% | **选择缺口 ~26pp@K64**(Pass 95 vs maj 67-71 且 K 大退化);官方: T=0.7 70.0% vs T=1.0 68.0%——温度判死;生成侧已解决,选择侧是唯一瓶颈 |
| **★B1 多实例 vav（2026-08-15，决定性胜利）** | 官方 **74.3%** | **新项目最优(768/1034)**。v1+v2 各 16 候选(vLLM 采样) → 全实例执行分组投票: 阶梯 多数66.8 → 单实例vav 67.2 → 2实例 71.5 → 全实例 74.3(+7.1pp);跨模型池 +2pp(72.2/71.8→74.3);超 v1-5p 70.1(+4.2pp);距 FINER 官方 76.6 仅 2.3pp 且无大规模 GRPO。官方-自评错位根因实证=单实例分组, 多实例分组根治。**L4 方法核心 MI-VAV 诞生** |
...
Host gf-bastion
  ...
  Port 11545                      # ★当前有效端口（2026-08-19 堡垒机 cpolar 重启回漂 14847→11545；根因=堡垒机监控任务停摆致公告未更新，已修复重启）
  User ASUS
...
| cpolar 地址/端口漂移（漂移史 10444→11220→10889→11545→14847→**11545**，2026-08-19 回漂） | 公告板机制（Gist `94fb05cee1ed4fe1b0b9edfbb3d718e2`）2026-08-14 首次实战闭环：堡垒机 AI 推送新端口 → 本地读公告板更新 config；TUNNEL_PORT_URL 环境变量已配，ssh_hpc_auto.ps1 支持断线自动恢复 |
...
- **随后**：训练侧 RL 已封盘（T2.1/2/3 三连负）。主线转向：① MPEV 官方复测（在跑）→ 诊断曲线（排队）→ memory 过滤/few-shot 变体 ② 蒸馏加料 SFT v2（3764 条 QC 教案 + gold 混合,预期单次 65-70%→投票 75%+）③ L7 机制论文数据同步积累。
